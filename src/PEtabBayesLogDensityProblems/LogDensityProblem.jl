LogDensityProblems.logdensity(p::PEtabBayesLogDensity, θ) = p.logtarget(θ)
...
LogDensityProblems.logdensity_and_gradient(p::PEtabBayesLogDensity, x) = p.logtarget_gradient(x)
...
function _logtarget(
    ...
    logtarget = PEtabBayes.compute_llh(x_inference, compute_nllh, inference_info)
    logtarget += PEtabBayes.compute_prior(x_inference, inference_info)
...
function _logtarget_gradient(
    ...
    # Logposterior with Jacobian correction for transformed parameters
    logtarget = nllh * -1 + PEtabBayes.compute_prior(x_inference, inference_info)
    logtarget += Bijectors.logabsdetjac(inference_info.inv_bijectors, x_inference)

    # Gradient with transformation correction
    PEtabBayes.correct_gradient!(logtarget_grad, x_inference, x_nllh, inference_info)
