...
Unlike the posterior `sample` methods, this draws directly and independently from the prior
distributions, so no sampler configuration or initial point `x0` is required.

# Arguments
- `rng`: Random number generator used to draw the samples. Defaults to
  `Random.default_rng()`.
...
function sample(
    ...
    # NOTE: draws are on the prior scale, matching the posterior `_to_chains_*` helpers.
    # Converting both to the PEtab parameter scale is deferred to a later PR.
